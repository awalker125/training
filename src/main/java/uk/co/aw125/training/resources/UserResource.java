...
@Path("/user")
@Api(value = "/user", description = "Operations about user", tags = {"Deprecated"})
...
public class UserResource {
  ...
  @ApiResponses(value = {@ApiResponse(code = 400, message = "Invalid user supplied"), @ApiResponse(code = 404, message = "User not found"),
      @ApiResponse(code = 400,
          message = "username specified in path does not match username in the user object. username is immutable. Please create a new user instead")})
  public Response updateUser(@ApiParam(value = "name that need to be deleted", required = true) @PathParam("username") String username,
    ...
    if (username.equals(user.getUsername())) {
      ...
      throw new CustomBadRequestException(
          "username specified in path does not match username in the user object. username is immutable. Please create a new user instead");
    }
  }
